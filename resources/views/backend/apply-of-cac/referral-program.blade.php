    ...
    <div class="content-wrapper">
        <div class="container-xxl flex-grow-1 container-p-y">
            <div class="col-xl-12">
                ...
                <!-- File input -->
                <form action="{{ route('referralprogram.update') }}" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="card">
                            ...
                            <div class="row">
                                ...
                                <div class="mb-3 col-12">
                                    <div id="jsonWrapper" class="space-y-3">
                                        {{-- Existing data --}}
                                        @if(isset($getContent->img_json) && isset($getContent->title_json))
                                            @php
                                            ...
                                            @foreach($images as $index => $image)
                                                <div class="border border-[#0F4634] p-3 relative jsonItem">
                                                    <div class="mb-2">
                                                        <img src="{{ asset($image) }}" width="150" class="rounded border previewImg">
                                                    </div>
                                                    <input type="file" name="img_json[]" class="form-control imgInput" accept="image/*">
                                                    <input type="hidden" name="old_img_json[]" value="{{ $image }}">
                                                    <div class="mt-2">
                                                        <textarea name="title_json[]" class="form-control" placeholder="Description">{{ $titles[$index] ?? '' }}</textarea>
                                                    </div>
