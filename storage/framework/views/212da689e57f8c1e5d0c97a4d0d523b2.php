...
<?php $__env->startSection('content'); ?>
    ...
    <div class="content-wrapper">
        <div class="container-xxl flex-grow-1 container-p-y">
            <div class="col-xl-12">
                <!-- File input -->
                <form action="<?php echo e(route('job-guaranteed-program.store')); ?>" method="post" enctype="multipart/form-data">
                    <?php echo csrf_field(); ?>
                    <div class="card">
                            ...
                            <div class="row">
                               <div class="mb-3 col-12">
                                ...
                                <div class="mb-5 col-12">
                                    <label class="block mb-2 text-sm font-medium text-[#0F4634]">Imge of Program</label>
                                    ...
                                    <div class="flex">
                                        <label class="uploader flex flex-col items-center justify-center w-[500px] h-[300px] rounded-lg
                                            ...
                                            <input type="file" name="img_card" accept="image/*"
                                                class="input-file absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
                                        </label>
                                    </div>
                                </div>

                                <h1 class="text-3xl text-[#0F4634]">Page Details</h1>
                                <div class="mb-3 col-12">
                                    <label for="formFile" class="form-label text-[#0F4634]">Description Of Program Details</label>
                                    <textarea name="desc_details" class="form-control" id="desc_details"></textarea>
                                </div>

                                <!-- ADD: no old image initially -->
                                <div class="mb-5 col-12">
                                    <label class="block mb-2 text-sm font-medium text-[#0F4634]">Imge of Program Details</label>

                                    <div class="flex">
                                        <label class="uploader-1 flex flex-col items-center justify-center w-[500px] h-[300px] rounded-lg
                                            border-2 border-dashed border-[#0F4634]/40 cursor-pointer bg-[#F9FAFB] hover:bg-[#F3F4F6]
                                            transition relative overflow-hidden">

                                            <!-- Preview (empty for add) -->
                                            <img class="preview-image-1 hidden absolute inset-0 m-auto w-[400px] h-[280px] object-cover rounded-lg"
                                                alt="preview" />

                                            <!-- Placeholder -->
                                            <div
                                                class="upload-placeholder-1 flex flex-col items-center justify-center text-center">
                                                <!-- svg -->
                                                <svg class="w-10 h-10 mb-2 text-[#0F4634]" fill="none" stroke="currentColor"
                                                    stroke-width="2" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                            ...
                                            <!-- Reset button (hidden for add until chosen) -->
                                            <button type="button"
                                                class="reset-btn-1 hidden absolute top-2 right-2 bg-white/80 rounded-full p-1 text-sm">✕</button>

                                            <!-- File input — NOT display:none. It covers the whole label and is clickable. -->
                                            <input type="file" name="img_details" accept="image/*"
                                                class="input-file-1 absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
                                        </label>
                                    </div>
                                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
  document.querySelectorAll('.uploader-1').forEach(uploader => {
    const inputFile = uploader.querySelector("input[type='file']");
    const previewImage = uploader.querySelector(".preview-image-1");
    const placeholder = uploader.querySelector(".upload-placeholder-1");
    const resetBtn = uploader.querySelector(".reset-btn");
...
<?php echo $__env->make('backend.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\Seed Media\Growth_Master\resources\views/backend/program-job/add-block-program.blade.php ENDPATH**/ ?>